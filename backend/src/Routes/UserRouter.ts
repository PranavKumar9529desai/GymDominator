...
UserRouter.post("completedtodaysprogress", async (c) => {
  ...
  try {
    ...
    const dayNumber = Math.floor(
      (date.getTime() - userProgress.enrolledDate.getTime()) /
        (1000 * 60 * 60 * 24)
    );
    ...
    const isCompletedTodayPlan: boolean =
      (userProgress[fieldToUpdate] & (1n << BigInt(bitPosition))) !== 0n;
    console.log("isCompletedTodayPlan", isCompletedTodayPlan);
    c.status(200);
    return c.json({
      msg: "sucesss",
      AlreadyCompletedTheplan: isCompletedTodayPlan,
    });
  } catch (error) {
    c.status(300);
    return c.json({ msg: "error" });
...
UserRouter.get("compltedDays", async (c) => {
  ...
  try {
    ...
    return c.json({
      ...
      enrolledDate: userProgress.enrolledDate,
      completionDate: userProgress.ComlpetionDate,
    });
  } catch (error) {
    console.log(error);
    c.status(300);
