import { Calendar, Trophy, ChevronLeft, ChevronRight, Check } from 'lucide-react';
...
export const MonthProgressComponent = () => {
  ...
  const [arrayOfCompletedDays, setArrayOfCompletedDays] = useState<Date[]>(defaultCompletedDays);
  ...
  const progressPercentage = (arrayOfCompletedDays.length / daysInMonth.length) * 100;
    ...
    <div className="w-full h-full lg:max-w-6xl mx-auto px-6 space-y-6 pb-40 lg:pb-0">
      ...
      <div>
        ...
        <div className="flex items-center justify-between lg:mt-8 my-4">
          ...
          <h2 className="text-xl font-semibold lg:my-2">{format(currentDate, 'MMMM yyyy')}</h2>
        ...
        <div className="grid grid-cols-7 gap-4 lg:gap-4">
          ...
          {daysInMonth.map((day) => (
            ...
            >
              {isCompleted(day) ? <div className="lg:text-4xl text-2xl">💪</div> : format(day, 'd')}
              {isCompleted(day) && <Check className="h-4 w-4 absolute top-1 right-1" />}
            </Button>
          ))}
        </div>
