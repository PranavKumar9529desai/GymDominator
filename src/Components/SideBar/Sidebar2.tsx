...
import type { Route } from '@components/Dashboard/PremiumUsersBNC';
...
import { FiCreditCard, FiHeart, FiUser } from 'react-icons/fi';
...
export const Sidebar2 = () => {
  ...
  const isActive = (item: Route) =>
    activeRoute === item.path || item.subRoutes?.some((subItem) => activeRoute === subItem.path);
  ...
  return (
    <div className="flex flex-col bg-slate-900 text-white w-64 h-screen ">
      ...
      <div className=" flex items-center justify-start py-6 px-4  w-fit  mx-auto space-x-2">
        <img src="/favicon.ico" alt="Gymnavigator Logo" className="rounded-full w-10 h-10" />
            ...
            <li key={item.name} className="whitespace-nowrap">
              <button
                ...
                className={`flex items-center w-full px-4 py-2 rounded-lg transition-colors duration-200 
                  ${isActive(item) ? 'bg-blue-700 text-white' : 'text-gray-300 hover:bg-gray-800'}`}
              >
      ...
      <div className="p-4 border-t border-gray-800 relative group">
        ...
        <div
          className="absolute bottom-full left-0 w-full p-4 bg-slate-900 rounded-t-lg border-t border-x border-gray-800 shadow-lg 
