    ...
    <div className="flex items-center gap-4">
      <button
        ...
        className={`p-2 rounded-full transition ${
          currentWeek === 1 ? 'text-gray-300' : 'text-gray-600 hover:bg-gray-100'
        }`}
      >
      ...
      <button
        ...
        className={`p-2 rounded-full transition ${
          currentWeek === totalWeeks ? 'text-gray-300' : 'text-gray-600 hover:bg-gray-100'
        }`}
      >
      ...
      <AnimatePresence>
        {isLocked ? (
          ...
          <div className="space-y-6">
            {mealTimes.map(
              ({ time, meal }) => meal && <MealCard key={time} time={time} meal={meal} />
            )}
          </div>
        )}
      </AnimatePresence>
